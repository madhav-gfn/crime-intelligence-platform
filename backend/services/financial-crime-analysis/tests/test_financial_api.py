...
def test_suspicious_accounts_high(client):
    r = client.get("/api/financial/suspicious-accounts", params={"risk_tier": "HIGH", "limit": 10})
    assert r.status_code == 200
    body = r.json()
...
def test_suspicious_accounts_invalid_tier(client):
    r = client.get("/api/financial/suspicious-accounts", params={"risk_tier": "CRITICAL"})
    assert r.status_code == 422


def test_account_profile_found(client):
    listing = client.get("/api/financial/suspicious-accounts", params={"risk_tier": "HIGH", "limit": 1}).json()
    account_id = listing["accounts"][0]["account_id"]
    r = client.get(f"/api/financial/account/{account_id}")
    assert r.status_code == 200
    assert r.json()["account_id"] == account_id


def test_account_profile_not_found(client):
    r = client.get("/api/financial/account/NOT_A_REAL_ACCOUNT_ID")
    assert r.status_code == 404


def test_patterns_all(client):
    r = client.get("/api/financial/patterns")
    assert r.status_code == 200
    body = r.json()
...
def test_patterns_filtered_by_typology(client):
    r = client.get("/api/financial/patterns", params={"typology": "FAN-OUT", "limit": 100})
    assert r.status_code == 200
    body = r.json()
...
def test_path_direct_edge(client):
    ...
    r = client.get("/api/financial/path", params={"source": edges["from_id"], "target": edges["to_id"]})
    assert r.status_code == 200
    body = r.json()
...
def test_path_no_route(client):
    r = client.get("/api/financial/path", params={"source": "NOT_REAL_A", "target": "NOT_REAL_B"})
    assert r.status_code == 200
    assert r.json()["found"] is False


def test_evaluate(client):
    r = client.get("/api/financial/evaluate")
    assert r.status_code == 200
    body = r.json()
