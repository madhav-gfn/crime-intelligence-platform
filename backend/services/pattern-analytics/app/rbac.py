...
This service never returns a person's name or any individually-identifying
field (hotspots are geospatial clusters, district-severity/trends/emerging
are district-level aggregates, similar-cases returns other fir_ids and
case metadata but no person names) - so every endpoint only requires the
`ANALYST` floor. Contrast with network-analysis/offender-profiling, where
most endpoints require `INVESTIGATOR` because they do surface names.
"""
import jwt
