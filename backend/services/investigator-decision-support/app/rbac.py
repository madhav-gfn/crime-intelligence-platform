...
Roles, least to most access:
    ANALYST      - required everywhere. Sufficient for /stats,
                   /case-priority, /case/{fir_id}, and /district-briefing -
                   none of those name a specific person, only fir_id/
                   district/aggregate fields.
    INVESTIGATOR - required for /person-dossier/{id}, which is the one
                   endpoint in this service that surfaces a full name,
                   co-accused associates, and case history for a specific
                   individual - clearly PII-adjacent.
