...
Roles, least to most access:
    ANALYST      - model-wide SHAP importance, methodology overview,
                   what-if predict-explain against a hypothetical profile
                   (no real person named)
    INVESTIGATOR - a specific person's real local SHAP explanation
                   (PII-adjacent, same bar offender-profiling itself uses
                   for /person/{id})
